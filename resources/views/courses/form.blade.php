    ...
    <div class="pl-5 pr-5">
        <form 
            ...
            <div class="row justify-content-center">
                                ...
                                <div class="col-md-6">
                                    <select name="level_id" id="level_id" class="form-control">
                                        @foreach (\App\Level::pluck('name', 'id') as $id => $level)
                                            <option 
                                                {{ old('level_id') === $id || $course->level_id === $id ? 'selected' : '' }} 
                                                value="{{ $id }}">
                                                {{ $level }}
                                            </option>
                                        @endforeach
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <select name="category_id" id="category_id" class="form-control">
                                        {{-- {{ Sin modo estrico (databases.php) }}
                                        @foreach ( \App\Category::all()->unique('name')->pluck('name', 'id') as $id => $category ) --}}

                                        {{-- {{ Quitando modo estricto (databases.php)}} --}}
                                        @foreach ( \App\Category::groupBy('name')->pluck('name', 'id') as $id => $category )
                                            <option 
                                                {{ old('category_id') === $id || $course->category_id === $id ? 'selected' : '' }} 
                                                value="{{ $id }}">
                                                {{ $category }}
                                            </option>
                                        @endforeach
                                    ...
                                    <div class="col-md-6">
                                        <input 
                                            ...
                                            value="{{ old('requirements.0') 
                                                    ? old('requirements.0') 
                                                    : ($course->requiriments_count > 0 
                                                    ? $course->requirements[0]->requirement 
                                                    : '') }}"
                                        >
                                ...
                                <label for="requirement2" class="col-md-4 col-form-label text-md-right">
                                    ...
                                    <div class="col-md-6">
                                        <input 
                                            ...
                                            value="{{ old('requirements.1') 
                                                    ? old('requirements.1') 
                                                    : ($course->requiriments_count > 0 
                                                    ? $course->requirements[1]->requirement 
                                                    : '') }}"
                                        >
                ...
                <div class="col-md-8">
                    <div class="card">
                        <div class="card-header">{{ __("¿Qué conseguira el estudiante al finalizar el curso?") }}</div>
